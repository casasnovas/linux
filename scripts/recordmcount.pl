...
$mcount_regex = "^\\s*([0-9a-fA-F]+):.*\\s(mcount|__fentry__|__sanitizer_cov_trace_pc)\$";
$section_type = '@progbits';
$mcount_adjust = 0;
$type = ".long";

if ($arch eq "x86_64") {
    $mcount_regex = "^\\s*([0-9a-fA-F]+):.*\\s(mcount|__fentry__|__sanitizer_cov_trace_pc)([+-]0x[0-9a-zA-Z]+)?\$";
    $type = ".quad";
    $alignment = 8;
    $mcount_adjust = -1;
